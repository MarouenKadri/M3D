...
namespace M3D_ISICG
{
	...
	void LabWork3::_initBuffers()
	{
		...
		glVertexArrayElementBuffer( _cube._vao, _cube._ebo );
			...
			std::cout << "hey From labwork 3" << std::endl;
	...
	{  
		...
		std::cout << "Hy from View Matrix " << std::endl;
	}
	...
	void LabWork3::_updateProjectionMatrix()
	{
		...
		std::cout << "hy from Projection Matrix" << std::endl;
...
_cube._vertices
		 ...
		 //_cube._transformation = glm::mat4( 1.0f );
		 //_cube._transformation = glm::rotate( _cube._transformation, glm::radians( 20.0f ), glm::vec3( 0.0f, 0.0f, 1.0f ) ); 
		 _cube._transformation   = glm::scale( _cube._transformation, Vec3f( 0.8f ) );
		 //glProgramUniformMatrix4fv( _program, _uModelMatrixLoc, 1, GL_FALSE, glm::value_ptr( _cube._transformation ) );
		 return _cube; 
